...
class ModuleToggle(BaseModel):
    ...
    def _normalize(cls, value: "ModuleToggle") -> "ModuleToggle":
        """Normalize extras ordering for deterministic serialization."""

        if value.extras:
            # Maintain tuple type but ensure stable ordering for rendering/tests.
            normalized = tuple(dict.fromkeys(value.extras))
            object.__setattr__(value, "extras", normalized)
        return value
...
class AppConfig(BaseModel):
    ...
    @model_validator(mode="after")
    def _post_process(cls, value: "AppConfig") -> "AppConfig":
...
def _coerce_toggle(value: Any) -> ModuleToggle:
    """Convert arbitrary module toggle representations to :class:`ModuleToggle`."""
...
def _normalize_modules(raw: Mapping[str, Any] | None) -> dict[str, ModuleToggle]:
...
def render_user_config(config: AppConfig, *, include_defaults: bool = True) -> str:
    ...
    if include_defaults:
        ...
        document.add(tomlkit.comment(
            "Precedence: CLI flags > environment variables > raggd.toml > packaged defaults"
        ))
